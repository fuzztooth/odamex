...
void STACK_ARGS M_SaveDefaults(std::string filename)
{
	...
	if (auto uf = uqFile(fopen(configfile.c_str(), "w")))
	{
		auto f = uf.get();

		fmt::print(f, "// Generated by Odamex " DOTVERSIONSTR " - don't hurt anything\n\n");

		// Archive all cvars marked as CVAR_ARCHIVE
		fmt::print(f, "// --- Console variables ---\n\n");
		cvar_t::C_ArchiveCVars(f);

		// Archive all active key bindings
		fmt::print(f, "// --- Key Bindings ---\n\n");
		fmt::print(f, "unbindall\n");
		Bindings.ArchiveBindings(f);
		DoubleBindings.ArchiveBindings(f);

		fmt::print(f, "\n// --- Automap Bindings ---\n\n");
		fmt::print(f, "unambind all\n");

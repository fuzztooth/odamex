...
void STACK_ARGS M_SaveDefaults(std::string filename)
{
	...
	if (auto f = uqFile(fopen(configfile.c_str(), "w")))
	{
		fmt::print(f.get(), "// Generated by Odasrv " DOTVERSIONSTR "\n\n");

		// Archive all cvars marked as CVAR_ARCHIVE
		fmt::print(f.get(), "// --- Console variables ---\n\n");
		cvar_t::C_ArchiveCVars(f.get());

		// Archive all aliases
		fmt::print(f.get(), "\n// --- Aliases ---\n\n");
		DConsoleAlias::C_ArchiveAliases(f.get());
...
bool M_ReadJSON(Json::Value &json, const std::string& filename)
{
	...
	if (length > 0 && buffer)
	{
		...
		if (res == false)
		{
			PrintFmt(PRINT_HIGH,"M_ReadJSONFromFile: Error parsing JSON: {}.\n",
